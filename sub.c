...
void displayTransactionList(Transaction *transactions, int transactionCount);
...
void displayMenu() {
  ...
  logToConsole("choosen", "12. Clear screen\n");
  logToConsole(
      "border",
      "=============================================================\n");
}
  ...
  while (1) {
    ...
    case 1: {
      do {
        ...
        if (findMaterialIndexById(materials, id, materialCount) != -1) {
          if (mode == 1) {
            transferMaterial(transactions, materials, transactionCount,
                             materialCount, id, mode, transID);
            break;
          } else {
            transferMaterial(transactions, materials, transactionCount,
                             materialCount, id, mode, transID);
...
void transferMaterial(Transaction **transactions, Material *materials,
  ...
  if (*transactionCount > MAX_LIST_SIZE) {
    printf(RED "Material list reached max size (%d). Cannot add more!" RESET,
           MAX_LIST_SIZE);
    return;
  }
  ...
  for (int i = 0; i < materialCount; i++) {
    if (strcasecmp(materials[i].matId, id) == 0) {
      if (type == 1) {
        ...
        // import
        readInt(&transCount,
                "Enter material count to import: ", "Materials count");
        ...
        do {
          ...
          readInt(&transCount,
                  "Enter material count to export: ", "Materials count");
